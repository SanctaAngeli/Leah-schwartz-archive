...
|---|---|---|
| Q1 | Leah's handwriting samples — do we have any to digitize? | **ANSWERED 2026-06-10:** the book's title page (`public/photos/p006_01.jpg`) is her actual signature with the subtitle in print. Used as the reader's masthead; could be cropped/vectorized for the lobby wordmark later. Caveat remains the body hand. |
...
- **2026-06-10 (v3.3)** — Sprint R3 "Art between the words" shipped. The reader (`/her-words`) is now a true book: a table of contents headed by her actual title-page signature, then one route per section — her autobiography split into HER OWN fourteen sections (IMMIGRANTS · *genetic baggage* …) with reading times, a progress hairline, and prev/next doors. The ProseFlow engine (`src/components/reader/`) enforces the rhythm rule — never more than ~5 paragraphs without a photograph, painting moment, or caption figure — placing the book's photographs at the very pages where she is in the story. Recovered content: her two-page PREFACE (pp. 14–15) transcribed from the scans into the Introduction ("…maybe a recipe or two (but no ARTSPEAK)"), four photo captions un-mangled from fake headings, and the broken "ART SCHOOL, LOVE AND MARRIAGE" heading repaired. Theme rooms now open on a hero painting with her essay interleaved between grid rows. At Her Age shows a curated ten per era with an expander (289-image dump gone). `/life` merged into the Her Story wing (same life, told once); LifePage and proseSections deleted. Q1 (handwriting) answered: p006_01.jpg is her real signature.
- **2026-06-10 (v3.2)** — Sprint R2 "The floor plan" shipped. The site is now one lobby + five wings (Paintings · Her Story · Places · Studio · About), defined in `src/data/wings.ts`. New wing landings at `/paintings` and `/her-story` adopt every orphaned signature page as a door card (Atlas gets a real palette-spectrum strip). The lobby shows the painting of the day (click opens it), her dates and one-line identity, a rotating verbatim pull-quote, a visible "Watch the intro" pill, and the five wings as the foot directory. Nav pill reduced from nine items to six, with wing-aware active state. A "Continue the walk" rail at the foot of every room chains all 19 rooms in curated order and returns to the lobby; fullscreen rooms (canvas, walk) get a floating pill. Studio table gained a Walk With Her hotspot; About gained the archive rail (Index · Constellation · Preservation · Favorites). Inbound-link audit: zero orphan routes (four had none before). R3 (the reading experience) is next, pending Harry's go.
- **2026-06-10 (v3.1)** — Sprint R1 "No broken rooms" shipped, per [SITE_REVIEW_2026-06.md](SITE_REVIEW_2026-06.md). Deleted six zombie v1 pages (`/eras` `/story` `/classic` `/timeline` `/tour` `/curated` — two showed the wrong birth year) with redirects to living equivalents; removed the duplicate legacy keyboard-shortcut system; fixed the production build, added the missing ESLint 9 config, and fixed five rules-of-hooks crashes it found. Registered the CSS-variable design tokens in Tailwind (active pills were rendering black-on-black sitewide; night mode now flows through every token). Removed the fake 2s preloader. Watercolor shaders now demand-render at ~25fps and pause when hidden. Mobile: artwork lightbox no longer collides with the nav pill and opens on the painting, not the info sheet. Content: OCR sweep of all 13 prose files (mirrored-page garbage, signage transcriptions, broken dimensions), travel regions promoted to real headings, duplicate chapter taglines stripped, caption-dumps no longer shown as "In Leah's own words", theme cards are real links, Autobiography card routes to the reader. R2 (floor plan) and R3 (reading experience) await Harry's sign-off.
